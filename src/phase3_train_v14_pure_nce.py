#!/usr/bin/env python3
"""
Phase 3 v14 — Pure InfoNCE Contrastive Learning (No Classification)
=========================================================================
Key changes over V13:
  - Removed CE classification loss entirely (competes with NCE for capacity)
  - Removed classifier head (fewer params, no gradient conflict)
  - Removed label smoothing, SMOTE, all lambda hyperparameters
  - Pure InfoNCE: model focuses 100% on embedding quality
  - Colder temperature: tau=0.05 (vs V13 tau=0.07) -> harder contrastive task
  - ReduceLROnPlateau scheduler (vs CosineAnnealing -> simpler, more robust)
  - Simplified pipeline: single loss, single objective, clean story

Rationale: V13 showed CE and NCE compete for model capacity. Top embedding
works (SimCLR, MoCo, SimCSE) all use pure contrastive learning. We follow
this proven paradigm: one loss, one goal — learn embeddings that cluster
same-label cases together and push different-label cases apart.
"V14 is what V13 wanted to be when it grew up."
"""
...
CONFIG = {
    ...
    'patience': 60,
    # ── InfoNCE ──
    'temperature': 0.05,       # COLD: harder task -> better embeddings
    # ── ReduceLROnPlateau ──
    'lr_factor': 0.5,
    'lr_patience': 15,
    'lr_min': 1e-6,
...
def json_graph_to_data_v14(graph_dict, graph_id=None, label=None, max_nodes=None):
    """
...
class GINEncoderV14(nn.Module):
    ...
    def forward(self, node_types, edge_index, edge_types, node_attrs, mask=None):
        """Pure encoder forward - returns L2-normalized embeddings only."""
...
# ═══════════════════════════════════════════════════════════
# PURE InfoNCE TRAINING (no CE, no classifier)
def train_nce(model, dataloader, optimizer, device):
    """Single training step: InfoNCE only."""
    model.train()
    total_loss_sum = 0.0
    num_batches = 0
    ...
    for batch in dataloader:
        ...
        embeddings = model(nt, ei, et, na, mask)
...
@torch.no_grad()
def validate_nce(model, dataloader, device):
    """Validation: InfoNCE loss only."""
    model.eval()
    total_loss_sum = 0.0
    num_batches = 0
...
def main():
    print("=" * 70)
    print("Phase 3 v14 — Pure InfoNCE (No Classification)")
    print("=" * 70)
    ...
    print(f"Loss: InfoNCE ONLY (tau={CONFIG['temperature']})")
    print(f"No classification head. No CE loss. No label smoothing.")
    print(f"Scheduler: ReduceLROnPlateau (factor={CONFIG['lr_factor']}, "
    ...
    for cid in train_ids + val_ids + test_ids:
        ...
        g = json_graph_to_data_v14(raw.get('graph', raw), graph_id=cid,
    ...
    # ── Build model (NO classifier) ──
    print("\n[3] Building GIN Encoder v14 (Pure Encoder, No Classifier)...")
    model = GINEncoderV14(num_node_types=len(NODE_TYPES), num_edge_types=len(EDGE_TYPES),
                          hidden_dim=CONFIG['hidden_dim'],
                          num_layers=CONFIG['num_layers'], dropout=CONFIG['dropout']).to(device)
    print(f"  Parameters: {sum(p.numel() for p in model.parameters()):,}")
    print(f"  Note: No classifier head — pure embedding encoder")

    # ═══ Pure InfoNCE Training ═══
    print("\n" + "=" * 70)
    print("[Training] Pure InfoNCE Contrastive Learning")
    print(f"  tau={CONFIG['temperature']} (cold → harder task)")
    print(f"  Loss = InfoNCE(embeddings, labels, tau)")
    print("=" * 70)
    ...
    hist = {'train_nce': [], 'val_nce': [], 'lr': []}

    for epoch in range(1, CONFIG['epochs'] + 1):
        # ── Train ──
        t_nce = train_nce(model, train_loader, optimizer, device)

        # ── Validate ──
        v_nce = validate_nce(model, val_loader, device)
        ...
        if is_best:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CONFIG['model_dir'], 'phase3_v14_pure_nce_best.pt'))
        else:
            patience_counter += 1
        ...
        # ── LR drop notification ──
        if epoch > 1 and lr_now < hist['lr'][-2]:
            print(f"  📉 LR dropped @ epoch {epoch} (→ {lr_now:.2e})")

        # ── Early stop ──
        if patience_counter >= CONFIG['patience']:
            print(f"  Early stop @ {epoch} (patience={CONFIG['patience']})")
            break
    ...
    emb_path = os.path.join(CONFIG['output_dir'], 'embeddings_v14_pure_nce.npz')
    ...
    for fname in ent_fnames:
        ...
        ent_graphs_loaded[eid] = json_graph_to_data_v14(raw, graph_id=eid,
    ...
    # ── Save results ──
    final_results = {
        'experiment': 'phase3_v14_pure_nce',
        'version': 'v14',
        'description': 'Pure InfoNCE contrastive learning — no classification head, no CE loss, cold temperature (tau=0.05)',
        'training': {
    ...
    with open(os.path.join(CONFIG['output_dir'], 'phase3_v14_pure_nce_results.json'), 'w') as f:
        json.dump(final_results, f, ensure_ascii=False, indent=2)
    torch.save({'model_state_dict': model.state_dict(), 'config': CONFIG,
                'unique_types': unique_types, 'type_to_label': type_to_label},
               os.path.join(CONFIG['model_dir'], 'phase3_v14_pure_nce_final.pt'))
    print("\n✅ Phase 3 v14 Complete!")
