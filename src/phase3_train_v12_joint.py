#!/usr/bin/env python3
"""
Phase 3 v12 — Joint Multi-Task Training (CE + InfoNCE)
=======================================================
Key change over V11:
  - Single-stage joint training: CE classification loss + InfoNCE contrastive loss
  - Weighted sum: total = λ_ce * CE + λ_nce * InfoNCE (default 0.3:0.7)
  - Label smoothing on CE loss (0.1) to prevent overconfident classifier
  - Same GINEncoderV11 architecture + NodeFeatureEncoder
  - SMOTE augmentation retained for InfoNCE only
  - Best model selected by validation InfoNCE loss

Rationale: V11 showed Phase A (classification) and Phase B (contrastive) have
conflicting objectives. Joint training balances both simultaneously, with
InfoNCE weighted higher since retrieval is the end goal.
"""
...
CONFIG = {
    ...
    'joint_patience': 50,
    # ── Loss weights ──
    'ce_weight': 0.3,        # classification weight
    'nce_weight': 0.7,       # InfoNCE contrastive weight
    'label_smoothing': 0.1,  # label smoothing for CE
    # ── InfoNCE ──
    'temperature': 0.5,
    # ── SMOTE ──
    'smote_interval': 10,
...
def json_graph_to_data_v12(graph_dict, graph_id=None, label=None, max_nodes=None):
    """
    Convert JSON graph to GraphData with node feature attributes.
    (Same as V11 — renamed for clarity)
    """
...
# ═══════════════════════════════════════════════════════════
# Node Feature Encoder (same as V11)
...
def smote_augment(embeddings, labels, k=5, alpha=0.3):
    device = embeddings.device
    unique_labels = torch.unique(labels)
    synthetic_embs, synthetic_labels = [], []
    for lbl in unique_labels:
        ...
        _, nn_idx = sim.topk(min(k + 1, n), dim=1)
        nn_idx = nn_idx[:, 1:]
        target_count = max(int(embeddings.shape[0] / len(unique_labels)), n)
        needed = min(max(0, target_count - n), n * 3)
        for _ in range(needed):
            i = random.randint(0, n - 1)
            j = nn_idx[i, random.randint(0, nn_idx.shape[1] - 1)]
            lam = alpha * random.random()
            synth = embs[i] + lam * (embs[j] - embs[i])
            synth = F.normalize(synth, p=2, dim=0)
            synthetic_embs.append(synth)
            synthetic_labels.append(lbl.item())
    if synthetic_embs:
        return torch.stack(synthetic_embs), torch.tensor(synthetic_labels, device=device)
    return torch.empty(0, embeddings.shape[1], device=device), torch.empty(0, device=device, dtype=torch.long)
...
def train_joint(model, dataloader, optimizer, device):
    ...
    Returns (avg_total_loss, avg_ce_loss, avg_nce_loss, ce_accuracy, all_embs, all_labels)
    """
    model.train()
    ...
    for batch in dataloader:
        ...
        # Collect for SMOTE
        all_embs.append(embeddings.detach())
    ...
    return (total_loss_sum / n, ce_sum / n, nce_sum / n,
            ...
            torch.cat(all_embs), torch.cat(all_labels))
...
def validate_joint(model, dataloader, device):
    ...
    Returns (avg_nce_loss, ce_accuracy, avg_ce_loss, all_embs, all_labels)
    """
    model.eval()
    ...
    if num_batches == 0:
        return float('inf'), 0.0, 0.0, None, None
...
def main():
    print("=" * 70)
    print("Phase 3 v12 — Joint Multi-Task (CE + InfoNCE) Training")
    print("=" * 70)
    ...
    for cid in train_ids + val_ids + test_ids:
        ...
        g = json_graph_to_data_v12(raw.get('graph', raw), graph_id=cid,
    ...
    # ── Build model ──
    print("\n[3] Building GIN Encoder v12 (Joint Training)...")
    model = GINEncoderV12(num_node_types=len(NODE_TYPES), num_edge_types=len(EDGE_TYPES),
    ...
    # ═══ Joint Training (CE + InfoNCE) ═══
    print("\n" + "=" * 70)
    print("[Joint Training] CE Loss + InfoNCE Contrastive + SMOTE")
    ...
    hist = {'train_total': [], 'train_ce': [], 'train_nce': [], 'train_acc': [],
            'val_nce': [], 'val_ce': [], 'val_acc': []}

    for epoch in range(1, CONFIG['joint_epochs'] + 1):
        # ── Train ──
        t_total, t_ce, t_nce, t_acc, te, tlb = train_joint(
            model, train_loader, optimizer, device)

        # ── Validate ──
        v_nce, v_acc, v_ce, ve, vlb = validate_joint(model, val_loader, device)
        ...
        # ── SMOTE augmentation (InfoNCE only) ──
        if epoch > 0 and epoch % CONFIG['smote_interval'] == 0 and te is not None:
            se, sl = smote_augment(te, tlb, k=CONFIG['smote_k'],
                                   alpha=CONFIG['smote_alpha'])
            if len(se) > 0:
                model.train()
                se = se.to(device); sl = sl.to(device)
                for _ in range(CONFIG['smote_finetune_steps']):
                    optimizer.zero_grad()
                    # Pass synthetic embeddings through projection for gradient flow
                    po = model.projection(se)
                    po = F.normalize(po, p=2, dim=1)
                    nce_loss_syn = info_nce_loss(po, sl, CONFIG['temperature'])
                    if torch.isfinite(nce_loss_syn) and nce_loss_syn.item() > 0:
                        nce_loss_syn.backward()
                        optimizer.step()
                print(f"  [SMOTE @{epoch}] +{len(se)} synth embeds")
        ...
        if is_best:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CONFIG['model_dir'], 'phase3_v12_joint_best.pt'))
        else:
            patience_counter += 1
    ...
    emb_path = os.path.join(CONFIG['output_dir'], 'embeddings_v12.npz')
    ...
    for fname in ent_fnames:
        ...
        ent_graphs_loaded[eid] = json_graph_to_data_v12(raw, graph_id=eid,
    ...
    with open(os.path.join(CONFIG['output_dir'], 'phase3_v12_joint_results.json'), 'w') as f:
        json.dump(final_results, f, ensure_ascii=False, indent=2)
    torch.save({'model_state_dict': model.state_dict(), 'config': CONFIG,
                'unique_types': unique_types, 'type_to_label': type_to_label},
               os.path.join(CONFIG['model_dir'], 'phase3_v12_joint_final.pt'))
    print("\n✅ Phase 3 v12 Complete!")
